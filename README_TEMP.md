...
- tau: `0.02`
- entropy coefficient: `0.02`
- conservative alpha: `0.1`
- expert KL weight: `0.02`
- smoothness weight: `0.01`
- cost budget: `0.0`
- Lagrange learning rate: `0.005`
- Lagrange initial value: `0.05`
- evaluation every: `10` epochs
...
- `smoothness_weight ∈ {0.005, 0.01, 0.02}`

This is important because the coursework required experimentation with key parameters rather than only one fixed run.

---

## 9. Final results

Two types of evidence are available:

1. the **main Kaggle experiment run** over five seeds;
...
- **CQL**: survival `0.7090`, inadmissibility `0.0909`, return `0.7090`
- **VOAC**: survival `0.6921`, inadmissibility `0.8406`, return `0.6921`
- **LAADAN-AC**: survival `0.7225`, inadmissibility `0.0000`, return `0.7225`
...
|---|---:|---:|---:|---:|---:|---:|---:|
| BC | 46 | 0.6965 | 0.3294 | 0.7148 | 0.4384 | 1.3262 | 3.3135 |
| CQL | 43 | 0.7195 | 0.4397 | 0.6528 | 5.9550 | 0.0000 | 3.9496 |
| VOAC | 43 | 0.6924 | 0.8418 | 0.2278 | 2.6467 | 3.1956 | 8.2445 |
| LAADAN-AC | 46 | **0.7285** | **0.0000** | **0.7972** | **0.2203** | 0.3482 | **1.2699** |
...
- **zero inadmissibility**,
- the **best expert argmax match**,
- the **lowest KL divergence to the expert**,
- and the **lowest mean action deviation**.

So the proposed design did not merely improve one metric while damaging the others. It produced the strongest overall trade-off.

---

## 10. Independent verification of the saved models

The script `test_final_models.py` was created to verify that the final saved checkpoints truly reproduce the recorded benchmark behaviour.

The test script does the following:

1. loads each final checkpoint,
2. rebuilds the architecture from `run_config.json`,
3. reconstructs the correct policy interpretation for each model,
4. evaluates the policy exactly on the benchmark,
5. compares recomputed metrics to the saved `metrics.json`,
6. and checks whether the differences remain within predefined tolerances.
...
Therefore, the reported saved metrics are **supported by the actual saved models**, not by unverified logging alone.
...
This README should stay honest about limitations.
...
icu-sepsis (2024) *The ICU-Sepsis Environment*. GitHub repository. Available at: [https://github.com/icu-sepsis/icu-sepsis](https://github.com/icu-sepsis/icu-sepsis) (Accessed: 12 May 2026).
...
Tessler, C., Mankowitz, D.J. and Mannor, S. (2019) Reward constrained policy optimization, in *Proceedings of the 7th International Conference on Learning Representations (ICLR 2019)*. Available at: [https://openreview.net/forum?id=SkfrvsA9FX](https://openreview.net/forum?id=SkfrvsA9FX) (Accessed: 12 May 2026).
